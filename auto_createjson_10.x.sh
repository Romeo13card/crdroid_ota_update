...
path=/raid0/crdroid10                                                             # Here you will need to specify the path to the crDroid source code folder
...
zip=$(basename $path/out/target/product/$device/crDroidAndroid-14.0-*-$device-*.zip)        # Here we get the package name with the extension .zip
nozip=$(basename $path/out/target/product/$device/crDroidAndroid-14.0-*-$device-*.zip .zip) # Here we get the package name without the extension .zip
recovery=$(basename $path/out/target/product/$device/recovery.img)
...
git add -A && git commit -m "Update autogenerated json $version for $device $date/$time" && git push
gh release create $nozip --notes "Automated release crDroid $version for $device $date/$time" $path/out/target/product/$device/$zip $path/out/target/product/$device/recovery.img
